...
# ------------------------------------------------------------------------------
  def plot_connectivities(self, c_xx, info, channels_names, used_channels_names=None, mode='functional',
                          ...
                          min_max_normalization_type=None,
                          thr=99,
                          fig_title='',  w_label=None, fb_label=None,
                          labels_params = {'ylabel_fonfamily':'serif', 'ylabel_fontsize':18,
                                     'ylabel_weight':1000, 'xlabel_fonfamily':'serif', 'xlabel_fontsize':18,
                                      'xlabel_weight':500, 'rotation':0, 'cl_size':16},
                          plot_channels=True, relevant_channels=True, channel_importance=True,
                          plot_channels_names=True, show_connectivity_colorbar=True,
                          show_topomap_colorbar=True,
                          figsize=(30,30), cmap_connectivities='hot', cmap_tplt='bone',
                          show=True, save=False, path='', format='png'):
             ...
             for time in np.arange(n_vwt):
                ...
                if plot_channels_names:
                    if relevant_channels:
                        ...
                        for ch1 in  np.unique(np.ravel(indx_pct)):
                                ax.annotate(info['ch_names'][ch1], xy=pos[ch1,:], xytext=(0,10), textcoords="offset points", fontsize=12, fontstyle='italic', fontweight='500', ha='center')
                    else:
