...
class CSP(TransformerMixin, BaseEstimator):
    ...
    def __init__(self, n_components=4, reg=None, log=None, cov_est="concat",
                 ...
                 cov_method_params=None, rank=None,PCov=False):
        """Init of CSP."""
        # Init default CSP
    ...
    def fit(self, X, y):
        ...
        if  self.PCov == False:                           
            ...
            epochs,P = X.shape
            n_channels = int((1+np.sqrt(1+8*P))/2)
        ...
        sample_weights = list()
        if self.PCov == True:
            #X =  np.asarray([(squareform(epoch)+np.eye(n_channels))/np.linalg.norm(squareform(epoch)+np.eye(n_channels),ord='fro') for epoch in X])
            #X =  np.asarray([(squareform(epoch)+np.eye(n_channels)*np.linalg.norm(squareform(epoch)+np.eye(n_channels),ord='fro')) for epoch in X])
            #X =  np.asarray([squareform(epoch)+np.eye(n_channels) for epoch in X])
            #X =  np.asarray([(squareform(epoch)+np.eye(n_channels))/(np.linalg.norm(squareform(epoch)+np.eye(n_channels),ord='fro')) for epoch in X])
            X =  np.asarray([squareform(epoch)+np.eye(n_channels) for epoch in X])
            for i in range(len(X)):
                if np.linalg.eigvals(X[i]).min()<0:
                    X[i] = X[i]+(np.eye(n_channels)*(abs(np.linalg.eigvals(X[i]).min())+1e-8))
            #X =  np.asarray([(squareform(epoch)+np.eye(n_channels)) for epoch in X])
        for class_idx, this_class in enumerate(self._classes):
            if self.cov_est == "concat":  # concatenate epochs
                if self.PCov == True:
                    covtmp = X[y == this_class,:,:]
                    cov = np.mean(covtmp,axis=0)
                else:
        ...
        #kk = np.eye(n_channels)*(abs(np.linalg.eigvals(covs.sum(0)).min())+1e-8)
        if n_classes == 2:
            #try:
            #eigen_values, eigen_vectors = linalg.eigh(covs[0], covs.sum(0)+kk)
            eigen_values, eigen_vectors = linalg.eigh(covs[0], covs.sum(0))
            #except:
            #eigen_values, eigen_vectors = linalg.eigh(covs[0], covs.sum(0)+10*np.diag(np.diag(covs.sum(0))))
            ...
            eigen_vectors = np.real(eigen_vectors.T)
        ...
        if self.PCov == True:
            #X = np.asarray([pick_filters @ epoch @ pick_filters.T for epoch in self.CovM])
            #X =  np.asarray([squareform(epoch) for epoch in X])
            X = np.asarray([pick_filters@ epoch @ pick_filters.T for epoch in X])
            #X = np.asarray([pick_filters@X[n,:,:] for n in range(len(X))])
            #X = np.asarray([np.linalg.norm(epoch,axis=1)/np.linalg.norm(epoch,axis=1).sum() for epoch in X])
            X = np.asarray([ np.diag(epoch)/np.trace(epoch) for epoch in X])
            #X = X.mean(axis=2)
        else:            
    ...
    def transform(self, X):
        ...
        if self.PCov == True:
            #Xr = np.zeros((len(X),len(pick_filters)))
            #for n in range(len(X)):
            #    Xtmp = np.dot(pick_filters,np.dot(X[n,:,:],pick_filters.T)) 
            #    Xtmp = np.diag(Xtmp)/np.trace(Xtmp)#X.mean(axis=2)
            #    Xr[n,:] = Xtmp
            #X=Xr
            epochs,P = X.shape
            n_channels = int((1+np.sqrt(1+8*P))/2)
            ...
            #X = np.asarray([np.linalg.norm(epoch,axis=1)/np.linalg.norm(epoch,axis=1).sum() for epoch in X])
            #X = np.asarray([(squareform(epoch)+np.eye(n_channels)) for epoch in X])
            #X = np.asarray([np.dot(pick_filters,np.dot(X[n,:,:],pick_filters.T)) for n in range(len(X))])
            X = np.asarray([np.diag(epoch)/np.trace(epoch) for epoch in X])
        else:
            X = np.asarray([np.dot(pick_filters, epoch) for epoch in X])
